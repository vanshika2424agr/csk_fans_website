...
function getConfig(m) {
  if (m.status === 'Completed') return m.win ? statusConfig.win : statusConfig.loss;
...
export default function Schedule() {
  ...
  const loadMatches = async () => {
    ...
    try {
      ...
      console.log('[Schedule] API response — matches received:', data?.length, data);
  ...
  if (loading) {
    return (
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '80px 24px', textAlign: 'center' }}>
        ...
        <p style={{ color: 'var(--text-muted)', marginTop: 16, fontFamily: "'Outfit',sans-serif" }}>
  ...
  if (error && matches.length === 0) {
    return (
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '80px 24px', textAlign: 'center' }}>
        ...
        <h2 style={{ fontFamily: "'Outfit',sans-serif", fontWeight: 700, color: 'var(--text)', marginBottom: 8 }}>
          Failed to Load Matches
        </h2>
        <p style={{ color: 'var(--text-muted)', marginBottom: 20 }}>{error}</p>
        <button className="btn-primary" onClick={loadMatches}>
          🔄 Retry
        </button>
      </div>
    );
  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto', padding: '60px 24px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(130px,1fr))', gap: 12, marginBottom: 36 }}>
        {[
          { label: 'Played',   value: wins + losses, color: 'var(--text)'    },
          { label: 'Won',      value: wins,           color: 'var(--green)'   },
          { label: 'Lost',     value: losses,         color: 'var(--red)'     },
          { label: 'Upcoming', value: upcoming,       color: 'var(--primary)' },
        ].map(s => (
          <div key={s.label} className="card" style={{ textAlign: 'center', padding: '20px 12px' }}>
            <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: '2.4rem', color: s.color, lineHeight: 1 }}>
              {s.value}
            </div>
            <div style={{ fontSize: '0.68rem', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.1em', marginTop: 4 }}>
              {s.label}
            </div>
          </div>
        ))}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        {list.length === 0 && (
          <div className="card" style={{ textAlign: 'center', padding: '48px 24px' }}>
            ...
            <h3 style={{ fontFamily: "'Outfit',sans-serif", fontWeight: 700, color: 'var(--text)', marginBottom: 8 }}>
              No matches found
            </h3>
            <p style={{ color: 'var(--text-muted)', fontSize: '0.88rem', marginBottom: 20 }}>
              {tab === 'All'
                ? 'Database is empty. Make sure the backend is running — it will auto-seed matches.'
                : `No ${tab.toLowerCase()} matches right now.`}
            </p>
            <button className="btn-primary" style={{ display: 'inline-flex' }} onClick={loadMatches}>
              🔄 Refresh
            </button>
          </div>
        )}
        ...
        {list.map((m, i) => {
          ...
          return (
            <div
              ...
              style={{ animationDelay: `${i * 60}ms`, animationFillMode: 'both', padding: 0, overflow: 'hidden' }}
            >
              <div style={{
                height: 3,
              ...
              <div className="match-card-layout" style={{ padding: '20px 24px', display: 'flex', flexWrap: 'wrap', gap: 16, alignItems: 'center' }}>
                ...
                <div style={{ minWidth: 90, textAlign: 'center', flexShrink: 0 }}>
                  <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: '1.5rem', color: 'var(--primary)', lineHeight: 1 }}>
                    {m.date}
                  </div>
                  <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', marginTop: 2 }}>
                    {m.day}
                  </div>
                ...
                <div style={{ flex: 1, minWidth: 200 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                    ...
                    <span style={{ fontSize: '0.7rem', fontWeight: 700, color: 'var(--text-muted)', padding: '2px 10px', background: 'var(--primary-subtle)', borderRadius: 99, border: '1px solid var(--border)' }}>
                      vs
                    </span>
                    <span style={{ fontFamily: "'Outfit',sans-serif", fontWeight: 800, fontSize: '1.05rem', color: isCSKAway ? 'var(--primary)' : 'var(--text)' }}>
                  ...
                  {m.result && (
                    <div style={{ marginTop: 8, fontSize: '0.78rem', fontWeight: 600, color: m.win ? 'var(--green)' : 'var(--red)' }}>
                      {m.win ? '✅' : '❌'} {m.result}
                    </div>
                  )}
                  {m.status === 'Upcoming' && (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 10, flexShrink: 0 }}>
                  <div style={{
                    ...
                    padding: '5px 12px', borderRadius: 99, fontSize: '0.68rem', fontWeight: 700,
